...
static Boolean criarArquivoDeResultado(const char *caminho) {
    ...
    fprintf(
        arquivo,
        "arquivo,idInstancia,n,h,tamanhoSubcubo,quantidadePaineis,custoConstrutiva,custoBuscaLocal,melhoriaSobreConstrutivaPercentual,tempoConstrutivaMs,tempoBuscaLocalMs,iteracoes,vizinhosAvaliados,vizinhosUmaTroca,vizinhosSubcubos,movimentosUmaTrocaAceitos,movimentosSubcubosAceitos,paineisAvaliados,maiorCardinalidadeAceita,movimentosQuatroOuMaisAceitos\n"
    );
    fclose(arquivo);
    return VERDADEIRO;
}

static Boolean adicionarResultado(const char *caminho,const Instancia *instancia,FatorH fatorH,Custo custoConstrutiva,Custo custoBuscaLocal,TempoComputacionalEmMilissegundos tempoConstrutivaMs,TempoComputacionalEmMilissegundos tempoBuscaLocalMs,const ResultadoBuscaLocalSubcubosParticao *resultadoBuscaLocal) {
    FILE *arquivo;
    ...
    if(custoConstrutiva > 0) {
        melhoriaPercentual = ((double) (custoConstrutiva - custoBuscaLocal) * 100.0) / ((double) custoConstrutiva);
    }
    ...
    fprintf(
        arquivo,
        "%s,%u,%u,%.1f,%u,%u,%llu,%llu,%.9f,%.9f,%.9f,%u,%u,%u,%u,%u,%u,%u,%u,%u\n",
        ...
        (unsigned int) controllerBuscaLocalSubcubosParticaoObterTamanhoDoSubcubo((*instancia).quantidadeDeTarefas),
        (unsigned int) controllerBuscaLocalSubcubosParticaoObterQuantidadeDePaineis((*instancia).quantidadeDeTarefas),
        ...
        (unsigned int) (*resultadoBuscaLocal).resultadoBase.quantidadeDeVizinhosAvaliados,
        ...
        (unsigned int) (*resultadoBuscaLocal).resultadoBase.quantidadeDeMelhoriasPorReinsercao,
        (unsigned int) (*resultadoBuscaLocal).resultadoBase.quantidadeDeMelhoriasPorTroca,
...
int main(int argc,char **argv) {
    ...
    if(argc != 5) {
        printf("Uso: experimento_busca_local_subcubos_particao.exe arquivo fator_h id_instancia sufixo\n");
        return 1;
    }
    ...
    if(snprintf(caminhoResultado,sizeof(caminhoResultado),"resultados/validacao_busca_local_subcubos_particao_%s.csv",argv[4]) < 0) {
        return 1;
    }
    ...
    inicio = gerenciadorDeTempoObterTempoAtualEmSegundos();
    if(controllerBuscaLocalSubcubosParticaoMelhorarSolucao(&instancia,fatorH,&solucaoConstrutiva,&solucaoBuscaLocal,&resultadoBuscaLocal) == FALSO) {
        liberarSolucao(&solucaoConstrutiva);
        liberarInstancia(&instancia);
        return 1;
    }
    ...
    if(custoBuscaLocal != resultadoBuscaLocal.resultadoBase.custoFinal || custoBuscaLocal > custoConstrutiva) {
    ...
    printf(
        "arquivo=%s | instancia=%u | n=%u | h=%.1f | subcubo=%u | paineis=%u | construtiva=%llu | busca_local=%llu | melhora=%.6f%% | tempo_ms=%.3f | iteracoes=%u | vizinhos=%u | simples=%u | subcubos=%u | max_k=%u | aceitos_k4_mais=%u\n",
        argv[1],
        ...
        (unsigned long long) custoConstrutiva,
        (unsigned long long) custoBuscaLocal,
        custoConstrutiva > 0 ? ((double) (custoConstrutiva - custoBuscaLocal) * 100.0) / ((double) custoConstrutiva) : 0.0,
        tempoBuscaLocalMs,
        (unsigned int) resultadoBuscaLocal.resultadoBase.quantidadeDeIteracoes,
        (unsigned int) resultadoBuscaLocal.resultadoBase.quantidadeDeVizinhosAvaliados,
        (unsigned int) resultadoBuscaLocal.resultadoBase.quantidadeDeMelhoriasPorReinsercao,
