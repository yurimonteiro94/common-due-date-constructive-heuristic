...
static Boolean montarInstancia(Instancia *instancia) {
    if(inicializarInstancia(instancia,"teste_troca_k_particao",1,8) == FALSO) {
        return FALSO;
    }
...
int main(void) {
    ...
    if(controllerBuscaLocalTrocaKParticaoMelhorarSolucao(&instancia,FATOR_H_04,&solucaoInicial,&solucaoFinal,&resultadoBuscaLocal) == FALSO) {
        printf("Falha na busca local por troca-k da particao.\n");
    ...
    if(resultadoBuscaLocal.quantidadeDeVizinhosPorReinsercao == 0 || resultadoBuscaLocal.quantidadeDeVizinhosPorTroca == 0) {
        printf("A vizinhanca troca-k nao avaliou movimentos simples e compostos.\n");
    ...
    printf("Teste da busca local por troca-k da particao aprovado.\n");
