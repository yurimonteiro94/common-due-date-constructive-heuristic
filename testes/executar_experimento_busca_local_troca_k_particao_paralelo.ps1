...
function Iniciar-Tarefa {
    ...
    $caminhoLog = Join-Path $diretorioProjeto (
        "resultados\validacao_busca_local_troca_k_particao_" +
        $Tarefa.Sufixo +
        ".log"
    )
    ...
    $trabalho = Start-Job -ScriptBlock {
        ...
        & ".\testes\experimento_busca_local_troca_k_particao.exe" `
            $arquivo `
            $fatorH `
            $instancia `
            $sufixo `
...
if($houveFalha) {
    Write-Host ""
    Write-Host "Uma ou mais execucoes da busca local por troca-k da particao falharam."
...
$quantidadeDeResultados = @(
    Get-ChildItem -Path (Join-Path $diretorioProjeto "resultados") `
        -Filter "validacao_busca_local_troca_k_particao_*.csv" `
        -File
).Count
...
Write-Host ""
Write-Host "Todas as execucoes da busca local por troca-k da particao terminaram com sucesso."
