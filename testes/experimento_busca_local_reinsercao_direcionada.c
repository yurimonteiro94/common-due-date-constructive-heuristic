#include "../controller/controller_busca_local_reinsercao_direcionada/controller_busca_local_reinsercao_direcionada.h"
...
static QuantidadeDeTarefas obterRaioLocal(QuantidadeDeTarefas quantidadeDeTarefas) {
    ...
    if(quantidadeDeTarefas <= LIMITE_REINSERCAO_DIRECIONADA_REPRESENTATIVA) {
        return RAIO_LOCAL_REINSERCAO_DIRECIONADA_REPRESENTATIVA;
    }

    return RAIO_LOCAL_REINSERCAO_DIRECIONADA_GRANDE;
}

static QuantidadeDeTarefas obterQuantidadeDeAncoras(QuantidadeDeTarefas quantidadeDeTarefas) {
    if(quantidadeDeTarefas > LIMITE_REINSERCAO_DIRECIONADA_GLOBAL && quantidadeDeTarefas <= LIMITE_REINSERCAO_DIRECIONADA_REPRESENTATIVA) {
        return QUANTIDADE_ANCORAS_REINSERCAO_DIRECIONADA_REPRESENTATIVA;
    }
...
static QuantidadeDeTarefas obterRaioDaFronteira(QuantidadeDeTarefas quantidadeDeTarefas) {
    if(quantidadeDeTarefas > LIMITE_REINSERCAO_DIRECIONADA_GLOBAL && quantidadeDeTarefas <= LIMITE_REINSERCAO_DIRECIONADA_REPRESENTATIVA) {
        return RAIO_FRONTEIRA_REINSERCAO_DIRECIONADA_REPRESENTATIVA;
    }

    if(quantidadeDeTarefas > LIMITE_REINSERCAO_DIRECIONADA_REPRESENTATIVA) {
        return RAIO_FRONTEIRA_REINSERCAO_DIRECIONADA_GRANDE;
    }

    return 0;
}

static QuantidadeDeTarefas obterQuantidadeDeDestinosEstruturais(QuantidadeDeTarefas quantidadeDeTarefas) {
    if(quantidadeDeTarefas > LIMITE_REINSERCAO_DIRECIONADA_REPRESENTATIVA) {
        return QUANTIDADE_DESTINOS_ESTRUTURAIS_REINSERCAO_DIRECIONADA;
    }
...
static Boolean criarArquivoDeResultado(const char *caminho) {
    ...
    fprintf(
        arquivo,
        "arquivo,idInstancia,n,h,tipoVizinhanca,raioLocal,quantidadeAncoras,raioFronteira,quantidadeDestinosEstruturais,custoConstrutiva,custoBuscaLocal,melhoriaSobreConstrutivaPercentual,tempoConstrutivaMs,tempoBuscaLocalMs,iteracoes,vizinhosAvaliados,reinsercoesAceitas\n"
    );
...
static Boolean adicionarResultado(const char *caminho,const Instancia *instancia,FatorH fatorH,Custo custoConstrutiva,Custo custoBuscaLocal,TempoComputacionalEmMilissegundos tempoConstrutivaMs,TempoComputacionalEmMilissegundos tempoBuscaLocalMs,const ResultadoBuscaLocal *resultado) {
    ...
    fprintf(
        arquivo,
        "%s,%u,%u,%.1f,%s,%u,%u,%u,%u,%llu,%llu,%.9f,%.9f,%.9f,%u,%u,%u\n",
        ...
        (unsigned int) obterQuantidadeDeDestinosEstruturais((*instancia).quantidadeDeTarefas),
...
int main(int argc,char **argv) {
    ...
    if(argc != 5) {
        printf("Uso: experimento_busca_local_reinsercao_direcionada.exe arquivo fator_h id_instancia sufixo\n");
    ...
    if(snprintf(caminhoResultado,sizeof(caminhoResultado),"resultados/validacao_busca_local_reinsercao_direcionada_%s.csv",argv[4]) < 0) {
        return 1;
    }
    ...
    if(controllerBuscaLocalReinsercaoDirecionadaMelhorarSolucao(&instancia,fatorH,&solucaoConstrutiva,&solucaoBuscaLocal,&resultadoBuscaLocal) == FALSO) {
        liberarSolucao(&solucaoConstrutiva);
        liberarInstancia(&instancia);
