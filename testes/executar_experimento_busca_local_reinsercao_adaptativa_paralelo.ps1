...
$arquivos = @(
    [PSCustomObject]@{Arquivo="sch1000.txt"; Quantidade=1000; Peso=100},
    [PSCustomObject]@{Arquivo="sch500.txt"; Quantidade=500; Peso=70},
    [PSCustomObject]@{Arquivo="sch200.txt"; Quantidade=200; Peso=45},
    [PSCustomObject]@{Arquivo="sch100.txt"; Quantidade=100; Peso=30},
    [PSCustomObject]@{Arquivo="sch50.txt"; Quantidade=50; Peso=20},
    [PSCustomObject]@{Arquivo="sch20.txt"; Quantidade=20; Peso=10},
    [PSCustomObject]@{Arquivo="sch10.txt"; Quantidade=10; Peso=5}
)
...
foreach($arquivo in $arquivos) {
    foreach($fatorH in $fatoresH) {
        for($identificadorDaInstancia = 1;$identificadorDaInstancia -le 10;$identificadorDaInstancia++) {
            ...
            if($fatorH.Argumento -eq "4") {
                $peso = $peso + 4
            }
            elseif($fatorH.Argumento -eq "2") {
                $peso = $peso + 3
            }
            elseif($fatorH.Argumento -eq "6") {
                $peso = $peso + 2
            }
            else {
                $peso = $peso + 1
