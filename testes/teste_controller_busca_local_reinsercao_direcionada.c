...
static Boolean montarInstancia(Instancia *instancia) {
    if(inicializarInstancia(instancia,"teste_reinsercao_direcionada",1,8) == FALSO) {
        return FALSO;
    }
...
int main(void) {
    ...
    if(controllerBuscaLocalReinsercaoDirecionadaMelhorarSolucao(&instancia,FATOR_H_04,&solucaoInicial,&solucaoFinal,&resultado) == FALSO) {
        printf("Falha na busca local por reinsercao direcionada.\n");
    ...
    printf("Teste da busca local por reinsercao direcionada aprovado.\n");
