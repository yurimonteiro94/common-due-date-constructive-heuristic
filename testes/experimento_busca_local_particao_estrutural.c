#include "../controller/controller_busca_local_particao_estrutural/controller_busca_local_particao_estrutural.h"
...
static Boolean criarArquivoDeResultado(const char *caminho) {
    ...
    fprintf(
        arquivo,
        "arquivo,idInstancia,n,h,limiteCandidatosIntercambio,custoConstrutiva,custoBuscaLocal,melhoriaSobreConstrutivaPercentual,tempoConstrutivaMs,tempoBuscaLocalMs,iteracoes,vizinhosAvaliados,vizinhosRealocacaoLado,vizinhosIntercambioLados,realocacoesAceitas,intercambiosAceitos\n"
    );
...
static Boolean adicionarResultado(const char *caminho,const Instancia *instancia,FatorH fatorH,Custo custoConstrutiva,Custo custoBuscaLocal,TempoComputacionalEmMilissegundos tempoConstrutivaMs,TempoComputacionalEmMilissegundos tempoBuscaLocalMs,const ResultadoBuscaLocal *resultadoBuscaLocal) {
    ...
    fprintf(
        arquivo,
        "%s,%u,%u,%.1f,%u,%llu,%llu,%.9f,%.9f,%.9f,%u,%u,%u,%u,%u,%u\n",
        ...
        (unsigned int) obterLimiteDeCandidatos((*instancia).quantidadeDeTarefas),
...
int main(int argc,char **argv) {
    ...
    if(argc != 5) {
        printf("Uso: experimento_busca_local_particao_estrutural.exe arquivo fator_h id_instancia sufixo\n");
    ...
    if(snprintf(caminhoResultado,sizeof(caminhoResultado),"resultados/validacao_busca_local_particao_estrutural_%s.csv",argv[4]) < 0) {
        return 1;
    }
    ...
    if(controllerBuscaLocalParticaoEstruturalMelhorarSolucao(&instancia,fatorH,&solucaoConstrutiva,&solucaoBuscaLocal,&resultadoBuscaLocal) == FALSO) {
        liberarSolucao(&solucaoConstrutiva);
        liberarInstancia(&instancia);
    ...
    printf(
        "arquivo=%s | instancia=%u | n=%u | h=%.1f | limite_intercambio=%u | construtiva=%llu | busca_local=%llu | melhora=%.6f%% | tempo_ms=%.3f | iteracoes=%u | vizinhos=%u | realocacoes=%u | intercambios=%u\n",
        argv[1],
