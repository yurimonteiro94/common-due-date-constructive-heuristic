...
static Boolean montarInstancia(Instancia *instancia) {
    if(inicializarInstancia(instancia,"teste_particao_estrutural",1,8) == FALSO) {
        return FALSO;
    }
...
int main(void) {
    ...
    if(controllerBuscaLocalParticaoEstruturalMelhorarSolucao(&instancia,FATOR_H_04,&solucaoInicial,&solucaoFinal,&resultadoBuscaLocal) == FALSO) {
        printf("Falha na busca local por particao estrutural.\n");
    ...
    if(resultadoBuscaLocal.quantidadeDeVizinhosPorReinsercao == 0 || resultadoBuscaLocal.quantidadeDeVizinhosPorTroca == 0) {
        printf("A vizinhanca fixa nao avaliou os dois tipos de alteracao da particao.\n");
    ...
    printf("Teste da busca local por particao estrutural aprovado.\n");
