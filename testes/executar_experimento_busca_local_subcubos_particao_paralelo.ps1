...
$arquivos = @(
    [PSCustomObject]@{Arquivo="sch1000.txt"; Quantidade=1000; Peso=1000},
    [PSCustomObject]@{Arquivo="sch500.txt"; Quantidade=500; Peso=500},
    [PSCustomObject]@{Arquivo="sch200.txt"; Quantidade=200; Peso=200},
    [PSCustomObject]@{Arquivo="sch100.txt"; Quantidade=100; Peso=100},
    [PSCustomObject]@{Arquivo="sch50.txt"; Quantidade=50; Peso=50},
    [PSCustomObject]@{Arquivo="sch20.txt"; Quantidade=20; Peso=20},
    [PSCustomObject]@{Arquivo="sch10.txt"; Quantidade=10; Peso=10}
)

$fatoresH = @(
    [PSCustomObject]@{Argumento="2"; Texto="0.2"; Sufixo="02"; Peso=4},
    [PSCustomObject]@{Argumento="4"; Texto="0.4"; Sufixo="04"; Peso=3},
    [PSCustomObject]@{Argumento="6"; Texto="0.6"; Sufixo="06"; Peso=2},
    [PSCustomObject]@{Argumento="8"; Texto="0.8"; Sufixo="08"; Peso=1}
)
...
foreach($arquivo in $arquivos) {
    foreach($fatorH in $fatoresH) {
        for($identificadorDaInstancia = 1;$identificadorDaInstancia -le 10;$identificadorDaInstancia++) {
            $sufixo = $arquivo.Quantidade.ToString() + "_" + $identificadorDaInstancia.ToString().PadLeft(2,"0") + "_" + $fatorH.Sufixo
            $tarefas.Add(
                [PSCustomObject]@{
                    ...
                    Sufixo=$sufixo
                    Peso=($arquivo.Peso * 10) + $fatorH.Peso + ((11 - $identificadorDaInstancia) / 100.0)
                }
            )
        }
    }
...
function Iniciar-Tarefa {
    ...
    $caminhoLog = Join-Path $diretorioProjeto ("resultados\validacao_busca_local_subcubos_particao_" + $Tarefa.Sufixo + ".log")
    $caminhoErro = Join-Path $diretorioProjeto ("resultados\validacao_busca_local_subcubos_particao_" + $Tarefa.Sufixo + ".log.erro")
    ...
    $trabalho = Start-Job -ScriptBlock {
        ...
        & ".\testes\experimento_busca_local_subcubos_particao.exe" `
            $arquivo `
            $fatorH `
            $instancia `
            $sufixo `
        ...
        [PSCustomObject]@{CodigoDeSaida=$LASTEXITCODE}
    } -ArgumentList `
        $diretorioProjeto,`
        $Tarefa.Arquivo,`
        $Tarefa.H,`
    ...
    Write-Host ("Iniciada: n=" + $Tarefa.Quantidade + " instancia=" + $Tarefa.Instancia + " h=" + $Tarefa.TextoH + " Job=" + $trabalho.Id)
...
while($fila.Count -gt 0 -or $trabalhosAtivos.Count -gt 0) {
    while($fila.Count -gt 0 -and $trabalhosAtivos.Count -lt $quantidadeMaximaDeTrabalhadores) {
        $trabalhosAtivos.Add((Iniciar-Tarefa -Tarefa $fila.Dequeue()))
    }
    ...
    for($indice = $trabalhosAtivos.Count - 1;$indice -ge 0;$indice--) {
        ...
        if($estado -eq "Completed" -or $estado -eq "Failed" -or $estado -eq "Stopped") {
            $retornoDoTrabalho = @(Receive-Job $item.Trabalho -ErrorAction SilentlyContinue)
            $codigoDeSaida = 1
            ...
            if($codigoDeSaida -eq 0) {
                ...
                Write-Host ("Concluida " + $quantidadeConcluida + "/" + $quantidadeTotal + ": n=" + $item.Tarefa.Quantidade + " instancia=" + $item.Tarefa.Instancia + " h=" + $item.Tarefa.TextoH)
                ...
                $houveFalha = $true
                $quantidadeComFalha++
                Write-Host ("Falha " + $quantidadeConcluida + "/" + $quantidadeTotal + ": n=" + $item.Tarefa.Quantidade + " instancia=" + $item.Tarefa.Instancia + " h=" + $item.Tarefa.TextoH + " codigo=" + $codigoDeSaida)
    ...
    if(($instanteAtual - $instanteUltimoProgresso).TotalSeconds -ge 10) {
        ...
        Write-Host ("Progresso: " + $quantidadeConcluida + "/" + $quantidadeTotal + " | sucesso=" + $quantidadeComSucesso + " | falhas=" + $quantidadeComFalha + " | ativos=" + $trabalhosAtivos.Count + " | fila=" + $fila.Count + " | tempo=" + $duracao.ToString("hh\:mm\:ss"))
        $instanteUltimoProgresso = $instanteAtual
    }
}

$duracaoTotal = (Get-Date) - $tempoInicial
...
if($houveFalha) {
    Write-Host ""
    Write-Host "Uma ou mais execucoes da busca local por subcubos da particao falharam."
    exit 1
}

$quantidadeDeResultados = @(Get-ChildItem -Path (Join-Path $diretorioProjeto "resultados") -Filter "validacao_busca_local_subcubos_particao_*.csv" -File).Count
...
Write-Host ""
Write-Host "Todas as execucoes da busca local por subcubos da particao terminaram com sucesso."
exit 0