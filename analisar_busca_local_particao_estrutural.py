...
PADRAO_ARQUIVO = re.compile(
    r"^validacao_busca_local_particao_estrutural_"
...
CAMINHO_RESULTADO_ANTERIOR = os.path.join(CAMINHO_RESULTADOS,"resumo_busca_local_reinsercao_direcionada.csv")
CAMINHO_RESUMO = os.path.join(CAMINHO_RESULTADOS,"resumo_busca_local_particao_estrutural.csv")
CAMINHO_RESUMO_N_H = os.path.join(CAMINHO_RESULTADOS,"resumo_busca_local_particao_estrutural_por_n_h.csv")
...
def resumir(resultados):
    ...
    return {
        ...
        "vizinhosRealocacao": sum(int(linha["vizinhosRealocacaoLado"]) for linha in resultados),
        "vizinhosIntercambio": sum(int(linha["vizinhosIntercambioLados"]) for linha in resultados),
        "realocacoes": sum(int(linha["realocacoesAceitas"]) for linha in resultados),
        "intercambios": sum(int(linha["intercambiosAceitos"]) for linha in resultados)
    }
...
def imprimir_resumo(titulo,resumo):
    ...
    print("Vizinhos por realocacao de lado: " + str(resumo["vizinhosRealocacao"]))
    print("Vizinhos por intercambio de lados: " + str(resumo["vizinhosIntercambio"]))
    print("Realocacoes aceitas: " + str(resumo["realocacoes"]))
    print("Intercambios aceitos: " + str(resumo["intercambios"]))
...
def imprimir_comparacao_anterior(resumo):
    ...
    print()
    print("Comparacao diagnostica com a reinsercao direcionada anterior")
...
def salvar_resultados_enriquecidos(resultados):
    campos = [
        ...
        "iteracoes",
        "vizinhosAvaliados",
        "vizinhosRealocacaoLado",
        "vizinhosIntercambioLados",
        "realocacoesAceitas",
        "intercambiosAceitos"
    ]
...
def main():
    ...
    if(len(resultados) == 0):
        print("Nenhum resultado da busca local por particao estrutural foi encontrado.")
    ...
    imprimir_resumo("Resumo geral da busca local por particao estrutural",resumir(resultados))
