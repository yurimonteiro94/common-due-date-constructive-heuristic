#include "controller_busca_local_reinsercao_adaptativa.h"
...
static void prepararPosicoesCandidatas(Boolean *posicoesCandidatas,QuantidadeDeTarefas quantidadeDeTarefas,QuantidadeDeTarefas posicaoOrigem,QuantidadeDeTarefas quantidadeDeTarefasAdiantadas) {
    QuantidadeDeTarefas posicao;
    ...
    if(quantidadeDeTarefas <= LIMITE_REINSERCAO_REPRESENTATIVA) {
        for(indiceDaAncora = 0;indiceDaAncora < QUANTIDADE_ANCORAS_REINSERCAO_REPRESENTATIVA;indiceDaAncora++) {
            posicaoDaAncora = (QuantidadeDeTarefas) ((((InteiroPositivoDe32Bits) indiceDaAncora) * ((InteiroPositivoDe32Bits) quantidadeDeTarefas - 1u)) / ((InteiroPositivoDe32Bits) QUANTIDADE_ANCORAS_REINSERCAO_REPRESENTATIVA - 1u));
            posicoesCandidatas[posicaoDaAncora] = VERDADEIRO;
        }

        for(deslocamento = -(InteiroComSinalDe32Bits) RAIO_LOCAL_REINSERCAO_REPRESENTATIVA;deslocamento <= (InteiroComSinalDe32Bits) RAIO_LOCAL_REINSERCAO_REPRESENTATIVA;deslocamento++) {
            marcarPosicao(posicoesCandidatas,quantidadeDeTarefas,(InteiroComSinalDe32Bits) posicaoOrigem + deslocamento);
        }

        for(deslocamento = -(InteiroComSinalDe32Bits) RAIO_FRONTEIRA_REINSERCAO_REPRESENTATIVA;deslocamento <= (InteiroComSinalDe32Bits) RAIO_FRONTEIRA_REINSERCAO_REPRESENTATIVA;deslocamento++) {
    ...
    for(deslocamento = -(InteiroComSinalDe32Bits) RAIO_LOCAL_REINSERCAO_GRANDE;deslocamento <= (InteiroComSinalDe32Bits) RAIO_LOCAL_REINSERCAO_GRANDE;deslocamento++) {
        marcarPosicao(posicoesCandidatas,quantidadeDeTarefas,(InteiroComSinalDe32Bits) posicaoOrigem + deslocamento);
    }
}
...
static Boolean procurarMelhorReinsercao(Solucao *solucaoCorrente,const Tarefa **tarefasPorIdentificador,DataDeEntregaComum dataDeEntregaComum,InteiroPositivoDe32Bits *temposPrefixados,Boolean *posicoesCandidatas,Custo custoCorrente,QuantidadeDeTarefas quantidadeDeTarefasAdiantadas,ResultadoBuscaLocal *resultadoBuscaLocal,QuantidadeDeTarefas *melhorPosicaoOrigem,QuantidadeDeTarefas *melhorPosicaoDestino,Custo *melhorCusto,Boolean *encontrouMelhoria) {
    ...
    for(posicaoOrigem = 0;posicaoOrigem < (*solucaoCorrente).quantidadeDeTarefas;posicaoOrigem++) {
        prepararPosicoesCandidatas(posicoesCandidatas,(*solucaoCorrente).quantidadeDeTarefas,posicaoOrigem,quantidadeDeTarefasAdiantadas);
